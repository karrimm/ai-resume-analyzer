...
const Upload = () => {
    ...
    const handleAnalyze = async ( {companyName, jobTitle, jobDescription, file} : {companyName: string, jobTitle: string, jobDescription: string, file: File} ) => {
        ...
        if(!imageFile.file) return setStatusText(`Failed to convert PDF to image! ${imageFile.error || ""}`);
    ...
    return ( <main className="bg-[url('/images/bg-main.svg')] bg-cover">
        ...
        <section className="main-section">
            <div className="page-heading py-16">
                <h1>Smart Feedback, Tailored for your Personal Resume</h1>
                {
                    isProcessing ? (
                    <>
                        <h2>{StatusText}</h2>
                        <img src="/public/images/resume-scan.gif" className=""/>
                    </>
                    ) : (
                        <h2>Drop your Resume for ATS Scoring</h2>
                    )
                }
                {
                    !isProcessing && (
